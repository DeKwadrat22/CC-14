...
public sealed partial class GoobCVars
{
    ...
    // NOTE: atmos.humanoid_throw_multiplier and atmos.monstermos_rip_tiles_min_pressure are now defined
    // in Content.Shared/CCVar/CCVars.Atmos.cs as part of the Claw-Command space wind port, with defaults
    // that match space/'s gameplay tuning (2f and 20f respectively). They are removed from here to
    // prevent the duplicate-registration error at startup. The Goob defaults (2f / 7500f) were never
    // referenced by any code path in this fork.
    ...
    // SiliconNpcUpdateTime ("silicon.npcupdatetime") is already defined upstream
    // in Content.Shared/CCVar/CCVars.NPC.cs — removed to prevent duplicate registration.
