...
public abstract partial class SharedMansusGraspSystem : EntitySystem
{
    ...
    [Dependency] private Content.Shared.StatusEffectNew.StatusEffectsSystem _newStatusEffect = default!;
        ...
        out bool triggerGrasp)
    {
        ...
        switch (heretic.CurrentPath)
        {
            case "Ash":
            {
                // _ClawCommand: upstream PR #43705 replaced TemporaryBlindnessSystem
                // with BlindnessSystem (new StatusEffectNew API). Forward via the new
                // system explicitly since the old _statusEffect is from the legacy namespace.
                _newStatusEffect.TryAddStatusEffectDuration(target,
            ...
            case "Blade":
            {
                if (grasp != null && heretic.PathStage >= 7 && _tag.HasTag(target, BladeBladeTag))
                {
                ...
                if (_backstab.TryBackstab(target, performer, Angle.FromDegrees(45d)))
                {
                    ...
                    _damage.TryChangeDamage(target,
                        new DamageSpecifier(_proto.Index(SlashDamage), 10),
            ...
            case "Lock":
            {
                ...
                _audio.PlayPredicted(new SoundPathSpecifier("/Audio/_ClawCommand/Heretic/hereticknock.ogg"),
                    target,
                    performer);
                break;
            }
            ...
            case "Rust":
            {
                ...
                {
                    _damage.TryChangeDamage(target,
                        new DamageSpecifier(_proto.Index(BruteDamage), 500),
                        ignoreResistances: true,
                        origin: performer
                        );
                }
