...
public sealed partial class CosmicRunesSystem : EntitySystem
{
    ...
    private void OnInteractUsing(Entity<HereticCosmicRuneComponent> ent, ref AfterInteractUsingEvent args)
    {
        ...
        // Detect "is this a bible" via PrayableComponent (shared, upstream).
        // Avoids needing the server-only BibleComponent + BibleUserComponent in shared code.
        // Triggers for anyone using a bible (chaplain check would need server-side mind lookup).
        if (!HasComp<Content.Shared.Prayer.PrayableComponent>(args.Used) ||
        ...
        _audio.PlayPredicted(new SoundPathSpecifier("/Audio/Effects/holy.ogg"), Transform(ent).Coordinates, args.User);
        EnsureComp<FadingTimedDespawnComponent>(ent).Lifetime = 0f;
        args.Handled = true;
    }
    ...
    private bool Teleport(Entity<HereticCosmicRuneComponent> ent, EntityUid user)
    {
        ...
        if (pulling != null)
            _pulling.TryStartPull(user, pulling.Value, puller);
