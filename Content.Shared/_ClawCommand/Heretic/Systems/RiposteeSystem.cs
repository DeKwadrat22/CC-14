...
public sealed partial class RiposteeSystem : EntitySystem
{
        ...
        RiposteData data)
    {
        ...
        // _melee.InRange is protected; do an inline distance check instead.
        var userXform = Transform(user);
        var targetXform = Transform(target);
        var inRange = userXform.MapID == targetXform.MapID &&
                      (userXform.WorldPosition - targetXform.WorldPosition).LengthSquared() <= weapon.Comp.Range * weapon.Comp.Range;

        if (_melee.AttemptLightAttack(user, weapon.Owner, weapon.Comp, target) && _net.IsServer && inRange)
        {
...
public sealed partial class RiposteUsedEvent(NetEntity user, NetEntity target, NetEntity weapon, RiposteData data)
    : EntityEventArgs
{
