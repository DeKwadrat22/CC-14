...
// _Shitmed targeting/damage removed; fork lacks Shitmed surgery system.
namespace Content.Shared._ClawCommand.Lavaland.Body;
...
public sealed partial class CursedHeartSystem : EntitySystem
{
    ...
    private void Damage(EntityUid uid)
    {
        ...
        _damage.TryChangeDamage(uid, new DamageSpecifier(_proto.Index(AirlossGroup), 50), true, false);
    ...
    private void OnPump(EntityUid uid, CursedHeartComponent comp, PumpHeartActionEvent args)
    {
        ...
        args.Handled = true;
        _audio.PlayGlobal(new SoundPathSpecifier("/Audio/_ClawCommand/Lavaland/heartbeat.ogg"), uid);
        // _Shitmed split-damage stripped: apply flat group damage instead.
        _damage.TryChangeDamage(uid, new DamageSpecifier(_proto.Index(BruteGroup), -5), true, false);
        _damage.TryChangeDamage(uid, new DamageSpecifier(_proto.Index(AirlossGroup), -5), true, false);
        _damage.TryChangeDamage(uid, new DamageSpecifier(_proto.Index(BurnGroup), -8), true, false);
    ...
    private void OnUseInHand(EntityUid uid, CursedHeartGrantComponent comp, UseInHandEvent args)
    {
        ...
        _audio.PlayGlobal(new SoundPathSpecifier("/Audio/_ClawCommand/Lavaland/heartbeat.ogg"), args.User);
