...
using Content.Shared._ClawCommand.Lavaland.Mobs;
using Content.Shared._ClawCommand.Lavaland.Weapons.Marker;
...
namespace Content.Shared._ClawCommand.Lavaland.Weapons.Block;
...
public abstract partial class SharedBlockChargeSystem : EntitySystem
{
    ...
    {
        // _ClawCommand Lavaland: fork's BeforeDamageChangedEvent has no CanBeCancelled flag; the event
        // only fires when cancellation is possible. We bail if there is no Origin to match against.
        if (args.Origin is not { } origin
            || !TryComp<BlockChargeComponent>(component.BlockingWeapon, out var blockComp)
            || !HasComp<FaunaComponent>(origin)
            || !blockComp.HasCharge)
            return;

        _popup.PopupPredicted(Loc.GetString("block-attack-notice", ("user", uid), ("blocked", origin)), uid, null);
