...
using Content.Shared._ClawCommand.Lavaland.Anger.Components;
using Content.Shared._ClawCommand.Lavaland.Megafauna.Events;
using Content.Shared._ClawCommand.Lavaland.MobPhases;
...
namespace Content.Shared._ClawCommand.Lavaland.Anger.Systems;
...
public sealed partial class AngerSystem : EntitySystem
{
    ...
    [Dependency] private DamageableSystem _damageable = default!; // fork: TotalDamage has [Access] restriction
    ...
    public void UpdateAggression(Entity<AngerComponent?, AggressiveComponent?, DamageableComponent?> ent)
    {
        ...
        var healthProgress = Math.Max((float) (_damageable.GetTotalDamage((ent.Owner, damage)) / (anger.TotalHp * healthMultiplier)), 0f);
