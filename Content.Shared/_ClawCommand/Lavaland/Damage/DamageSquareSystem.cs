...
using Content.Shared._ClawCommand.Lavaland.Damage.Components;
// _Shitmed Targeting removed for fork (no per-part targeting)
...
namespace Content.Shared._ClawCommand.Lavaland.Damage;
    ...
    private void Damage(Entity<DamageSquareComponent> field)
    {
        ...
        foreach (var target in lookup)
        {
            if (!_damageQuery.TryComp(target, out var damageable)
                ...
                || (field.Comp.DamageBlacklist != null && _whitelist.IsValid(field.Comp.DamageBlacklist, target))) // fork: IsBlacklistPass not available
                continue;
            ...
            if (_net.IsServer) // Movement prediction is wonky and doesn't compensate for lag
            {
                ...
                _damage.TryChangeDamage((target, damageable),
                    field.Comp.Damage,
                    origin: field.Owner); // _Shitmed targetPart dropped; fork uses Entity<DamageableComponent?> shape
            }
