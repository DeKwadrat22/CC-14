...
namespace Content.Server._ClawCommand.Lavaland.Pressure;

public sealed partial class PressureEfficiencyChangeSystem : EntitySystem
{
    [Dependency] private AtmosphereSystem _atmos = default!;
    // _ClawCommand Lavaland: Shitmed/Woundmed targeting + SharedBodySystem.ConvertTargetBodyPart is not in this fork.
    // Removed the per-part trauma lookup and per-coverage filter; the modifier is now applied as a flat
    // damage multiplier on the wearer when their pressure condition is met.
    ...
    {
        // _ClawCommand Lavaland: fork's DamageModifyEvent has no TargetPart and DamageSpecifier has no
        // ArmorPenetration. We approximate ExtraPenetrationModifier as a flat multiplier applied to the
        // incoming damage before SharedArmorSystem reduces it — the wearer effectively takes more
        // (or less) damage when their pressure condition is met.
        if (!HasComp<ArmorComponent>(ent))
            return;
        ...
        args.Args.Damage *= 1f + ent.Comp.ExtraPenetrationModifier;
    }
