...
public sealed partial class HereticAbilitySystem
{
    ...
    private Entity<StomachComponent, OrganComponent> ResolveStomach(Entity<FleshPassiveComponent> ent)
    {
        ...
        // and removed EnsureAllSolutions entirely. MaxVol now needs prototype/post-create config.
        _solution.EnsureSolution(fleshStomach, solName, out _);
        stomach = EnsureComp<StomachComponent>(fleshStomach);
        // Upstream StomachComponent has no DigestionDelay; default upstream stomach pacing applies.
        organ = EnsureComp<OrganComponent>(fleshStomach);
        // Upstream OrganComponent has no IntegrityCap/OrganIntegrity (Goob _Shitmed surgery extension);
        // the eldritch stomach just exists, unbreakable in practice because it's never targeted.
        // OrganComponent.Body is restricted to BodySystem upstream — body-organ link gets set up
        // by the standard body-system flow when this organ is added to a body.
        Dirty(fleshStomach, organ);
        // Upstream MetabolizerComponent restricts most fields to MetabolismSystem.
        // The eldritch stomach uses upstream defaults instead of Goob's tuned values.
        EnsureComp<MetabolizerComponent>(fleshStomach);
