...
[Virtual] public partial class RitualSacrificeBehavior : RitualCustomBehavior
{
    ...
    public override void Finalize(RitualData args)
    {
        ...
        for (var i = 0; i < Max && i < uids.Count; i++)
        {
            ...
            // The sacrifice is consumed by the Mansus — no dismemberment, just total
            // eldritch annihilation. Apply lethal damage so the victim is unambiguously
            // dead, then queue them for deletion so the body doesn't linger on the rune.
            var consumed = new DamageSpecifier();
            consumed.DamageDict.Add("Slash", 500);
            consumed.DamageDict.Add("Heat", 500);
            consumed.DamageDict.Add("Asphyxiation", 500);
            args.EntityManager.System<DamageableSystem>().TryChangeDamage(uid, consumed, true);
