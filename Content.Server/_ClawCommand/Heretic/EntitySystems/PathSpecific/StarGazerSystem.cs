...
public sealed partial class StarGazerSystem : SharedStarGazerSystem
{
    ...
    {
        // CanReturnGlobal is upstream-read-only; the StarGazer can still ghost,
        // but they keep the ability to return to their body (no preventing that).
        // Could be restored by extending upstream GhostAttemptHandleEvent.
    }
    ...
    private void OnReset(Entity<CosmosPassiveComponent> ent, ref ResetStarGazerConsciousnessEvent args)
    {
        ...
        // GhostRoleComponent.Taken is restricted to GhostRoleSystem upstream — register
        // the role and let the system manage availability. IgnoreMindCheck is Goob-only;
        // upstream just respawn-checks via a different path.
    ...
    private void RemoveGhostRole(Entity<StarGazerComponent, GhostRoleComponent?> ent, bool hasMind, bool resettingMind)
    {
        ...
        // Taken is restricted upstream; UnregisterGhostRole takes the role out of the
        // ghost-role pool, which is equivalent semantically.
