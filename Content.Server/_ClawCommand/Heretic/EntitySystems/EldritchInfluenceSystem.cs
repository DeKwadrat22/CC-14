...
using Content.Shared._Goobstation.Wizard.Apprentice;
...
public sealed partial class EldritchInfluenceSystem : EntitySystem
{
    ...
    private void OnExamine(Entity<EldritchInfluenceComponent> ent, ref ExaminedEvent args)
    {
        ...
        // Upstream EntityEffect API uses RaiseEvent(target, raiser, scale, user) instead of
        // the legacy ShouldApply/Effect pair. The SharedEntityEffectsSystem implements IEntityEffectRaiser.
        var effects = _random.Pick(ent.Comp.PossibleExamineEffects);
        foreach (var effect in effects)
        {
            effect.RaiseEvent(args.Examiner, _effect, 1f, args.Examiner);
        }
    }
