...
using Content.Server._ClawCommand.HereticAdapters;
...
public sealed partial class HereticSystem : SharedHereticSystem
{
        ...
        bool playSound = true)
    {
        ...
        ChatSystemHereticExtensions.UpdateFontSize(size, ref message, ref loc);
    ...
    {
        // Upstream uses HumanoidProfileComponent, not Goob's HumanoidProfileComponent
        // with MarkingSet/hair/etc. We use a minimal profile (name + species) so the
        // sacrifice tracker still works; the heretic ritual UI will show a generic
        // silhouette instead of an exact appearance copy.
    ...
    private void OnAscension(Entity<HereticComponent> ent, ref EventHereticAscension args)
    {
        ...
        var ascendSound =
            new SoundPathSpecifier($"/Audio/_ClawCommand/Heretic/Ambience/Antag/Heretic/ascend_{pathLoc}.ogg");
