...
public sealed partial class ProtectiveBladeSystem : EntitySystem
{
    ...
    public static readonly SoundSpecifier BladeBlockSound =
        new SoundPathSpecifier("/Audio/_ClawCommand/Heretic/parry.ogg");
    ...
    {
        // Upstream ReflectSystem.TryReflectProjectile signature differs from Goob's.
        // For now: any protective-blade-equipped heretic just cancels the projectile
        // (treats it as caught/blocked) and consumes one blade per intercepted shot.
        if (args.Cancelled)
            return;

        foreach (var blade in GetBlades(ent))
        {
            if (!HasComp<ReflectComponent>(blade))
                continue;
    ...
    {
        // Upstream HitScanReflectAttemptEvent doesn't carry Damage; we set Reflected to
        // true with direction reversed so the shot bounces back at the shooter.
        if (args.Reflected)
            return;
        ...
        Vector2 target)
    {
        ...
        // upstream has no SharedGunSystem.SetTarget homing API; blade projectile flies straight.
