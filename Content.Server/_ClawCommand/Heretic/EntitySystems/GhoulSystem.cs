...
public sealed partial class GhoulSystem : EntitySystem
{
    ...
    // [Dependency] private HumanoidAppearanceSystem _humanoid = default!; // No upstream skin-color API
    ...
    public void GhoulifyEntity(Entity<GhoulComponent> ent)
    {
        ...
        // Upstream HumanoidAppearanceSystem has no SetSkinColor/SetBaseLayerColor public API;
        // ghouls don't get visually greyed out here. Could be restored by extending the
        // upstream humanoid system with those methods.
    ...
    private void SendBriefing(Entity<HereticMinionComponent?> ent)
    {
        ...
        var sound = new SoundPathSpecifier("/Audio/_ClawCommand/Heretic/Ambience/Antag/Heretic/heretic_gain.ogg");
    ...
    private void OnMobStateChange(Entity<GhoulComponent> ent, ref MobStateChangedEvent args)
    {
        ...
        // Eldritch consumption — instead of dismembering, the ghoul collapses into
        // a heretic-stained puddle. Spill all the blood, dust the husk, leave a Mansus mark.
        ...
        {
            _puddle.TrySpillAt(ent.Owner, bloodSolution.SplitSolution(bloodSolution.Volume), out _);
        }

        var ashed = new DamageSpecifier();
        ashed.DamageDict.Add("Heat", 1000);
        ashed.DamageDict.Add("Slash", 1000);
        _damageable.TryChangeDamage(ent.Owner, ashed, true);

        // Drop the ghoul's items as fallback so equipment is still recoverable.
        if (ent.Comp.DropOrgansOnDeath)
