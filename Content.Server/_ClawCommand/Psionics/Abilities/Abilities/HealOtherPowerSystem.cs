...
using Content.Shared.Traits.Assorted; // Claw Command - UnrevivableComponent lives here now


namespace Content.Server.Abilities.Psionics;

public sealed partial class RevivifyPowerSystem : EntitySystem
{
    ...
    private void OnDoAfter(EntityUid uid, PsionicComponent component, PsionicHealOtherDoAfterEvent args)
    {
        ...
        // Claw Command - TryChangeDamage now takes an Entity<DamageableComponent?> and no longer accepts the
        // component as a separate argument; origin moved up one slot.
        if (args.HealingAmount is not null)
            _damageable.TryChangeDamage((args.Target.Value, damageableComponent), args.HealingAmount * args.ModifiedAmplification, true, false, uid);

        // Claw Command - DamageableComponent.TotalDamage is no longer readable from outside DamageableSystem (RA0002).
        if (!args.DoRevive
            ...
            || _damageable.GetTotalDamage((args.Target.Value, damageableComponent)) > threshold)
            return;
    ...
    private void ActivatePower(EntityUid uid, PsionicComponent component, PsionicHealOtherPowerActionEvent args)
    {
        ...
        // Claw Command - see the matching note in OnDoAfter above.
        if (args.HealingAmount is not null)
            _damageable.TryChangeDamage((args.Target, damageableComponent), args.HealingAmount * args.ModifiedAmplification, true, false, uid);

        if (!args.DoRevive
            || HasComp<UnrevivableComponent>(args.Target) // Floofstation - unrevivable
            ...
            || _damageable.GetTotalDamage((args.Target, damageableComponent)) > threshold)
            return;
