...
public sealed partial class OracleSystem : EntitySystem
{
    ...
    [Dependency] private TelepathicChatSystem _tChat = default!;
    ...
    private void OnInteractUsing(Entity<OracleComponent> oracle, ref InteractUsingEvent args)
    {
        ...
        if (HasComp<MobStateComponent>(args.Used) || !TryComp(args.Used, out MetaDataComponent? meta) || meta.EntityPrototype == null)
            return;
    ...
    private void DispenseRewards(Entity<OracleComponent> oracle, EntityCoordinates throwTarget)
    {
        foreach (var rewardRandom in oracle.Comp.RewardEntities)
        {
            ...
            var reward = SpawnNextToOrDrop(rewardProto, oracle);
    ...
    private bool GetRandomTechProto(Entity<OracleComponent> oracle, [NotNullWhen(true)] out string? proto)
    {
        ...
        // Claw Command - ResearchSystem's server lookups are now scoped to a client entity and already filter to
        // the caller's own grid, which is exactly what upstream was doing by hand with GetServerIds() plus a
        // GridUid comparison. Asking for the Oracle's servers directly replaces both steps.
        var database = _research.GetServers(oracle.Owner)
            .Select(x =>
            {
                TryComp<TechnologyDatabaseComponent>(x.Owner, out var comp);
                return new Entity<TechnologyDatabaseComponent?>(x.Owner, comp);
            })
            .Where(x => x.Comp != null)
            .OrderByDescending(x =>
