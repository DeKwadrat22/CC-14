...
public sealed partial class PsionicsSystem : EntitySystem
{
    ...
    [Dependency] private PsionicFamiliarSystem _psionicFamiliar = default!;
    [Dependency] private NPCRetaliationSystem _retaliationSystem = default!;
    [Dependency] private AlertsSystem _alerts = default!;

    // Claw Command - RA0033: these parameters forbid literals, so the ids live in fields.
    ...
    public override void Initialize()
    {
        ...
        // Claw Command - upstream's TakeStaminaDamageEvent no longer exists. StaminaMeleeHitEvent is its
        // equivalent: raised on the weapon, before the damage is applied, with the same Multiplier/FlatModifier hooks.
        ...
        // Claw Command - upstream handled this through an IAlertClick implementation; alerts now raise an event.
        SubscribeLocalEvent<PsionicComponent, CheckManaAlertEvent>(OnCheckManaAlert);
    ...
    /// <summary>
    ///     Reports the psion's exact mana figure to them when they click the mana alert.
    /// </summary>
    private void OnCheckManaAlert(EntityUid uid, PsionicComponent component, CheckManaAlertEvent args)
    {
        if (args.Handled || !_playerManager.TryGetSessionByEntity(uid, out var session))
            return;

        args.Handled = true;

        var message = Loc.GetString("mana-alert", ("mana", MathF.Round(component.Mana)), ("manaMax", MathF.Round(component.MaxMana)));
    ...
    private void OnInit(EntityUid uid, PsionicComponent component, ComponentStartup args)
    {
        ...
        // Claw Command - ContainsFaction became IsMember, which takes an Entity<NpcFactionMemberComponent?>
        // instead of a trailing component argument.
        if (!component.Removable
            || !TryComp<NpcFactionMemberComponent>(uid, out var factions)
            || _npcFactonSystem.IsMember((uid, factions), GlimmerMonsterFaction))
            return;

        _npcFactonSystem.AddFaction(uid, PsionicInterloperFaction);
    }
    ...
    /// <summary>
    ///     Anti-psionic weapons hit psions harder in the stamina bar than they hit anyone else.
    /// </summary>
    /// <remarks>
    ///     Claw Command - upstream fired once per target via TakeStaminaDamageEvent.Target. StaminaMeleeHitEvent
    ///     instead carries the whole hit list and the resulting damage is divided evenly across it, so the bonus is
    ///     applied once if any psion was hit. For the single-target melee swings these weapons actually make, that
    ///     is identical to upstream behaviour.
    /// </remarks>
    private void OnStamHit(EntityUid uid, AntiPsionicWeaponComponent component, StaminaMeleeHitEvent args)
    {
        foreach (var (target, _) in args.HitList)
        {
