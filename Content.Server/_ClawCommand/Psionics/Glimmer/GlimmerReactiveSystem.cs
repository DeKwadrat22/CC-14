...
namespace Content.Server.Psionics.Glimmer
{
    public sealed partial class GlimmerReactiveSystem : EntitySystem
    {
        ...
        private void OnMeleeThrowOnHitAttempt(Entity<SharedGlimmerReactiveComponent> ent, ref AttemptMeleeThrowOnHitEvent args)
        {
            ...
            // Claw Command - AttemptMeleeThrowOnHitEvent.User is nullable now; nothing to zap if there is no user.
            if (args.User is not { } user)
                return;

            var zapTarget = _transformSystem.GetParentUid(user);
    ...
    /// </summary>
    // Claw Command - RA0003: must be explicitly sealed.
    public sealed partial class GlimmerTierChangedEvent : EntityEventArgs
    {
        /// <summary>
