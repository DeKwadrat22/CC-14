...
public sealed partial class TelepathicChatSystem : EntitySystem
{
    ...
    // Claw Command - fully qualified: Content.Shared.StatusEffect and Content.Shared.StatusEffectNew both
    // declare a type called StatusEffectsSystem, and the psionics port uses the old one elsewhere.
    [Dependency] private Shared.StatusEffectNew.StatusEffectsSystem _statusEffects = default!;

    /// <summary>
    ///     Claw Command - the hallucination status effect granted by space drugs. Anyone under it hears
    ///     telepathic chat as garbled dream-noise.
    /// </summary>
    private static readonly EntProtoId SeeingRainbowsEffect = "StatusEffectSeeingRainbow";
    ...
    private List<INetChannel> GetDreamers(IEnumerable<INetChannel> removeList)
    {
        ...
        var filtered = Filter.Empty()
            .AddWhereAttachedEntity(entity =>
                ...
                // Claw Command - SeeingRainbowsComponent is now SeeingRainbowsStatusEffectComponent and lives on a
                // separate status-effect entity rather than on the drugged player, so this asks the status effect
                // system instead of the player's own components. Same meaning: people tripping on space drugs
                // faintly pick up telepathic chatter.
                || _statusEffects.HasStatusEffect(entity, SeeingRainbowsEffect) && !HasComp<PsionicsDisabledComponent>(entity) && !HasComp<PsionicInsulationComponent>(entity))
            .Recipients
            .Select(p => p.Channel);
