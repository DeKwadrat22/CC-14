...
public sealed partial class BatteryDrinkerSystem : EntitySystem
{
    ...
    private void OnDoAfter(EntityUid uid, BatteryDrinkerComponent drinkerComp, DoAfterEvent args)
    {
        ...
        var drinkerCharge = _battery.GetCharge(drinkerBatteryComponent.Value.AsNullable());
        var drinkerMaxCharge = drinkerBatteryComponent.Value.Comp.MaxCharge;

        amountToDrink = MathF.Min(amountToDrink, sourceCharge);
        amountToDrink = MathF.Min(amountToDrink, drinkerMaxCharge - drinkerCharge);
