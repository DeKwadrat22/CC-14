...
namespace Content.Client._ClawCommand.Heretic.UI;
...
public sealed partial class LivingHeartMenu : RadialMenu
{
    ...
    private void UpdateUI()
    {
        ...
        foreach (var target in heretic.SacrificeTargets)
        {
            if (!_ent.TryGetEntity(target.Entity, out var ent) || !_ent.EntityExists(ent))
                // Upstream LobbyUIController has no LoadProfileEntity helper for spawning
                // a humanoid preview from a profile. Without it we skip this target's
                // avatar — the menu still shows the name/job in the tooltip.
                continue;
    ...
    public sealed partial class EmbeddedEntityMenuButton : RadialMenuButtonWithSector
    {
